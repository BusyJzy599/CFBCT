...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, csv_path = 'dataset_csv/ccrcc_clean.csv', mode = 'omic', apply_sig = False, 
        ...
        if self.apply_sig:
            ...
            self.signatures = pd.read_csv('./dataset_csv/pathway.csv')
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str='train', scaler=None):
        ...
        split = split[split.isin(self.return_h5_splits())].dropna().reset_index(drop=True)
    ...
    def return_h5_splits(self):
        h5_files=os.listdir(os.path.join(self.data_dir,"path_h5"))
        h5_files=[x[:-3] for x in h5_files]
    ...
    def getnoiseid(self):
        noise_dict={}
        for c in self.slide_data['slide_id']:
            noise_dict[string_hash(c)]=[
                0.0, # cindex
                0.0  # loss
            ]
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    with h5py.File(os.path.join(data_dir,'path_h5', f"{case_id}.h5"), 'r') as hf:
                        path_features = torch.FloatTensor(hf['data'][:])
                    cluster_ids=np.load(os.path.join(data_dir,'cluster_npy', f"{case_id}.npy"))
                    cluster_ids = torch.Tensor(cluster_ids)
                    genomic_features = torch.tensor(self.genomic_features.iloc[idx])
                    return (path_features, cluster_ids, torch.zeros((1,1)), label, event_time, c)
